...
Program:
  TypeDefBlock ClassDefBlock GDeclarations FdefBlock MainBlock { root = $5; }
  ;
...
ClassDef:
  ID 
  {
    currentClassEntry = installClass($1->varname);
    currentUdefType = currentClassEntry->type;
  }
  LBRACE DECL ClassFieldDecList MethodDecList ENDDECL MethodDefList RBRACE
  {
...
E:
  ...
  | FieldAccess
  | SELF_KW { $$ = makeSelfNode(); }
  | SELF_KW DOT ID LPAREN ArgList RPAREN { $$ = makeMethodCallNode(makeSelfNode(), $3->varname, $5); }
  | SELF_KW DOT ID LPAREN RPAREN { $$ = makeMethodCallNode(makeSelfNode(), $3->varname, NULL); }
