    ...
    while (temp) 
    {
        ...
        if (temp->paramlist != NULL) 
        {
            ...
            printf("│  Return Type: %s\n", 
                   ...
                   temp->type == TYPE_USERDEF ? temp->typeEntry->name : 
                   "unknown");
            printf("│  Function Label: F%d\n", temp->flabel);
            printf("│  Parameters: ");
            if(temp->paramlist->type==TYPE_NULL)
            {
                printf("No arguments");
            }
            else
        ...
        else 
        {
            ...
            else if(temp->type==TYPE_USERDEF)
            {
...
void printLST() {
    ...
    if (temp == NULL) {
        ...
        while (temp) {
            ...
            else if(temp->type==TYPE_USERDEF && temp->typeEntry)
            {
                typeName = temp->typeEntry->name;
            }
            else
...
int CompareParamLists(struct Paramstruct *list1, struct Paramstruct *list2) {
    ...
    while (list1 && list2) 
    {
        if((list1->type == TYPE_USERDEF || list2->type == TYPE_USERDEF) && (list1->typeEntry != list2->typeEntry))
        {
            printf("memory.c: Type mismatch in parameter list(udeftype)\n");
            return 0;
        }
