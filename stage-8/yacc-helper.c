...
{
    if (typeData->type == TYPE_CLASS)
    {
        // printf("hellooooo\n");
        struct Gsymbol *entry;
        Install(id->varname, TYPE_CLASS, 2);
        entry = Lookup(id->varname);
        entry->typeEntry = typeData->typeEntry;
    }
    else if (typeData->type == TYPE_USERDEF)
    {
...
int verifyMethodSignature(struct ClassTableEntry *classEntry, char *methodName, struct TypeInfo *retType, struct Paramstruct *params)
{
    if(!classEntry) {
        yyerror("Class not found");
        exit(1);
    }
    MethodEntry *method = lookupMethod(classEntry, methodName);
    if(!method) {
        yyerror("Method not found");
        exit(1);
    }
    if((retType->type == TYPE_USERDEF || retType->type == TYPE_CLASS) && method->retTypeEntry != retType->typeEntry) {
        yyerror("Method definition does not match declaration(udeftype or class)");
        exit(1);
    }
    if (!CompareParamLists(method->params, params)) 
    {
        // printf("method->type: %d, retType->type: %d, method->type != retType->type: %d\n", method->type, retType->type, method->type != retType->type);
        yyerror("Method definition does not match declaration");
        exit(1);
    }
    return 1;
}
